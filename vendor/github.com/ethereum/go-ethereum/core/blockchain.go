...
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, logs []*types.Log, state *state.StateDB) error {
	...
	if bc.cacheConfig.TrieDirtyDisabled {
		...
		if current := block.NumberU64(); current > TriesInMemory {
			...
			if nodes > limit || imgs > 4*1024*512 {
...
func CacheReceipts(bc *BlockChain, block *types.Block, receipts types.Receipts ){
	bc.receiptsCache.Add(block.Hash(), receipts)
}

func (bc *BlockChain) MyInsertChain(chain types.Blocks, Receipts types.Receipts) (int, error) {
	...
	defer bc.chainmu.Unlock()
	return bc.MyinsertChain(chain, true, true, Receipts)
}
func (bc *BlockChain) MyinsertChain(chain types.Blocks, verifySeals, setHead bool, Receipts types.Receipts) (int, error) {
	...
	for ; block != nil && err == nil || errors.Is(err, ErrKnownBlock); block, err = it.next() {
		...
		substart := time.Now()
		receipts, logs, usedGas, err := bc.processor.MyProcess(block, statedb, bc.vmConfig, Receipts)
		if err != nil {
...
// Inser
//tChain attempts to insert the given batch of blocks in to the canonical

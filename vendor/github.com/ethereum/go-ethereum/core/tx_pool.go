...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	if pool.currentState.GetBalance(from).Cmp(tx.Cost()) < 0 {
		fmt.Println("Insufficient Balance here")
...
func (pool *TxPool) add(tx *types.Transaction, local bool) (replaced bool, err error) {
	...
	validateStart := time.Now().UnixNano()/1000000
	// If the transaction fails basic validation, discard it
	if err := pool.validateTx(tx, isLocal); err != nil {
		log.Trace("Discarding invalid transaction", "hash", hash, "err", err)
		invalidTxMeter.Mark(1)
		validateEnd := time.Now().UnixNano()/1000000
		validateElapsed = validateElapsed + (validateEnd - validateStart)
		fmt.Println("Time spent on validation:", validateElapsed)
		return false, err
	}
...
func (pool *TxPool) runReorg(done chan struct{}, reset *txpoolResetRequest, dirtyAccounts *accountSet, events map[common.Address]*txSortedMap) {
	...
	if len(events) > 0 {
		...
		//from here the pending promoted transactions would be notified to the protocol manager
		//that broadcasts the txs to other peers
